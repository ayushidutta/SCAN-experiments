...
def generate_dataset(write_file_path, read_file_path):
    read_file_src = read_file_path + '.in'
    read_file_trg = read_file_path + '.out'
    write_file_src = write_file_path + '.in'
    write_file_trg = write_file_path + '.out'
    ...
    with open(write_file_src, 'w') as wfsrc, open(write_file_trg, 'w') as wftrg, \
            open(cl_src_files[0], 'w') as c1src, open(cl_trg_files[0], 'w') as c1trg, \
        ...
        for i, trg_line in enumerate(trg_lines):
            wfsrc.write(src_lines[i]+'\n')
            wftrg.write(trg_line+'\n')
            seq_len = len(trg_line.split(' '))
            if seq_len < 10:
...
for i, subdir in enumerate(data_subdirs):
    ...
    for scan_file in scan_files:
        ...
        generate_dataset(os.path.join(data_dir, subdir+'CL', file_name), os.path.join(data_dir, subdir, file_name))
